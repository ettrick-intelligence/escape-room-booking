...
class PaginationTest extends TestCase {
    ...
    public function test_invalid_page_size_falls_back_to_default() {
        $p = new EERB_Pagination( 100, 99, 1 ); // 99 is not a valid page size
        $this->assertEquals( EERB_Pagination::DEFAULT_PER_PAGE, $p->get_per_page() );
    }

    public function test_valid_page_sizes_accepted() {
        foreach ( EERB_Pagination::PAGE_SIZES as $size ) {
            $p = new EERB_Pagination( 100, $size, 1 );
    ...
    public function test_render_contains_showing_text() {
        $p = new EERB_Pagination( 100, 25, 2 );
        $html = $p->render( 'https://example.com/admin?page=eerb-bookings' );
    ...
    public function test_render_active_page_size_marked() {
        $p    = new EERB_Pagination( 100, 50, 1 );
        $html = $p->render( 'https://example.com' );
